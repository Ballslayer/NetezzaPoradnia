...
QSqlError initDatabase(QSqlDatabase &db) {
    ...
    //ta linijka ponizej chyba nawet nie potrzebne, bo dziala bez
    //db.setHostName("192.168.2.3");
    db.setDatabaseName("tera");
    db.setUserName("dbc");
    db.setPassword("dbc");
    ...
    QString query = "SELECT nr_diagnozy, nr_wizyty, nr_jednostkichorobowej, kod_leku, dawkowanie FROM diagnoza";
    qDebug() << query;
    if (!q.exec(query))
         qDebug() << q.lastError();
